...
class AdManager:
    ...
    async def create_unique_sale_time_idx(self):
        await self.published_posts.create_index(
            [('sale_msg_id', 1), ('time', 1), ("channel_owner_id", 1)],
            unique=True,
    ...
    async def check_old_published_ads_and_delete(self):
        ...
        async for ad in all_ads:
            ...
            if current_datetime > deletion_date:
                channel_owner_id = ad.get('channel_owner_id')
                ...
                for chat_id, message_ids in ad['published_posts'].items():
                    ...
                    try:
                        await self.delete_post(chat_id, message_ids, channel_owner_id, sale_msg_id, post_time)

                    except Exception as e:
                        logging.error(f"Error occurred: {e}")
                        logging.error(f"sale msg id {sale_msg_id} time {post_time}, owner id {channel_owner_id}")

                    if await self.all_posts_deleted(channel_owner_id, sale_msg_id, post_time):
                        await self.delete_from_published_sales(channel_owner_id, sale_msg_id, post_time)

    async def all_posts_deleted(self, channel_owner_id, sale_msg_id: int, post_time: str):
        doc = await self.published_posts.find_one(
            {'channel_owner_id': channel_owner_id, 'sale_msg_id': sale_msg_id, 'time': post_time})
    ...
    async def delete_post(self, from_chat, msg_ids: list[int], channel_owner_id: int, sale_msg_id, post_time):
        ...
        try:
            ...
            logging.info(f'{e} {sale_msg_id}, {post_time} owner: {channel_owner_id}')
            return

        for msg_id in msg_ids:
            await self.delete_from_published_posts(from_chat, msg_id, channel_owner_id, sale_msg_id, post_time)

        if await self.published_posts_deleted(from_chat, channel_owner_id, sale_msg_id, post_time):
            await self.published_posts.update_one(
                {'channel_owner_id': channel_owner_id, 'sale_msg_id': sale_msg_id, 'time': post_time},
                {'$unset': {'published_posts': from_chat}})

    async def published_posts_deleted(self, chat_id: int, channel_owner_id: int, sale_msg_id: int, time: str):
        doc = await self.published_posts.find_one(
            {'channel_owner_id': channel_owner_id, 'sale_msg_id': sale_msg_id, 'time': time})
        if not doc:
            return True
    ...
    async def register_published_ad(self, channel_owner_id: int, sale_msg_id: int, time: str, autodelete_timer: int):

        if await self.sale_is_published(channel_owner_id, sale_msg_id, time):
            return
    ...
    async def sale_is_published(self, channel_owner_id: int, sale_msg_id: int, time: str) -> bool:
        doc = await self.published_posts.find_one(
            {'channel_owner_id': channel_owner_id, 'sale_msg_id': sale_msg_id, 'time': time})
        r = doc is not None
        return r
    ...
    async def add_to_published_posts(self, channel_owner_id, sale_msg_id: int, time: str, chat_id: int,
                                     msg_ids: List[int]):
    ...
    async def delete_from_published_posts(self, from_chat: int, msg_id: int, channel_owner_id: int, sale_msg_id: int,
                                          time: str):
        await self.published_posts.update_one(
            {'channel_owner_id': channel_owner_id, 'sale_msg_id': sale_msg_id, 'time': time},
            {'$pull': {f'published_posts.{from_chat}': msg_id}})

    async def delete_from_published_sales(self, channel_owner_id: int, sale_msg_id, time):
        await self.published_posts.delete_one(
            {'channel_owner_id': channel_owner_id, 'sale_msg_id': sale_msg_id, 'time': time})
    ...
    async def update_info(self):
        await self.published_posts.update_many({}, {"$set": {"channel_owner_id": "936845322"}})
