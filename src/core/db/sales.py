...
class Sales:
    ...
    async def get_sales_for_specified_date(self, owner_id: int, dt: datetime):
        day, month, year = dt.day, dt.month, dt.year
        date: str = '{}-{}-{}'.format(str(day), str(month), str(year))
        sales = self.col.find({'channel_owner_id': owner_id, 'date': date})
        r = [sale async for sale in sales]
        return r
    ...
    async def get_closer_sale_id(self, owner_id: int, post_datatime: datetime):

        today_sales = await self.get_sales_for_specified_date(owner_id, datetime.now())
        yesterday_sales = await self.get_sales_for_specified_date(owner_id, datetime.now() - timedelta(days=1))
        tomorrow_sales = await self.get_sales_for_specified_date(owner_id, datetime.now() + timedelta(days=1))
    ...
    async def get_closest_sales(self, owner_id: int, post_datatime: datetime, time_period_in_minutes: int) -> list:
        time_period_in_seconds = time_period_in_minutes * 60
        today_sales = await self.get_sales_for_specified_date(owner_id, datetime.now())
        yesterday_sales = await self.get_sales_for_specified_date(owner_id, datetime.now() - timedelta(days=1))
        tomorrow_sales = await self.get_sales_for_specified_date(owner_id, datetime.now() + timedelta(days=1))
